...
/**
 * POST /api/campaigns/:campaignId/completion
 * Set completion evidence for a campaign (creator only)
 *
 * @param campaignId - Campaign ID (path parameter)
 * @header X-Wallet-Address - Creator's wallet address (required)
 * @body { evidenceId: string } - Evidence ID to set as completion
 * @returns {ApiResponse<{ campaignId: string; completionEvidenceId: string }>}
 */
export default defineEventHandler(async (event) => {
  try {
    ...
    const { evidenceId } = await parseBody(event, completionBodySchema)
    ...
    if (campaign.creator_address.toLowerCase() !== walletAddress.toLowerCase()) {
      throw createApiError(
        ...
        'Only the campaign creator can set completion evidence',
      )
    }
    ...
    await DB.batch([
      DB.prepare(`UPDATE evidence SET evidence_type = 'completion' WHERE evidence_id = ?`).bind(
        evidenceId,
      ),
      DB.prepare(`UPDATE campaigns SET completion_evidence_id = ? WHERE campaign_id = ?`).bind(
        evidenceId,
        campaignId,
      ),
    ])

    // TODO: Trigger CRE verification for completion evidence
    // This should call the CRE service to verify the completion evidence
    // against the baseline. For now, verification is manual/separate.
